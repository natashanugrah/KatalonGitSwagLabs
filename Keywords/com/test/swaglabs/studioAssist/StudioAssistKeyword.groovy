package com.test.swaglabs.studioAssist
...
public class StudioAssistKeyword {
	 ...
	 */
/*
 *  Clicks on a specified test object and handles exceptions accordingly.
 *
...
def ClickElement(String testObjectId) {
    try {
        // Clicks the element identified by the test object ID
        WebUI.click(findTestObject(testObjectId))
        // Marks the keyword as succeeded if the click is successful
        KeywordUtil.markPassed("Successfully clicked the element: " + testObjectId)
    } catch (WebElementNotFoundException e) {
        // Handles the case where the element cannot be found
        KeywordUtil.markFailed("Cannot find the element: " + testObjectId)
    } catch (Exception e) {
        // Handles any other exceptions that occur during the click action
